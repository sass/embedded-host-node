...
describe('request tracker', () => {
  ...
  describe('tracking requests', () => {
    it('tracks when empty', () => {
      tracker.add(0, OutboundMessage.MessageCase.COMPILERESPONSE);
      expect(tracker.nextId).toBe(1);
    });

    it('tracks multiple requests', () => {
      tracker.add(0, OutboundMessage.MessageCase.COMPILERESPONSE);
      tracker.add(1, OutboundMessage.MessageCase.COMPILERESPONSE);
      tracker.add(2, OutboundMessage.MessageCase.COMPILERESPONSE);
  ...
  describe('resolving requests', () => {
    it('resolves a single request', () => {
      tracker.add(0, OutboundMessage.MessageCase.COMPILERESPONSE);
      tracker.resolve(0, OutboundMessage.MessageCase.COMPILERESPONSE);
      expect(tracker.nextId).toBe(0);
    });
    ...
    it('resolves multiple requests', () => {
      ...
      tracker.add(1, OutboundMessage.MessageCase.COMPILERESPONSE);
      tracker.add(2, OutboundMessage.MessageCase.COMPILERESPONSE);
      tracker.resolve(1, OutboundMessage.MessageCase.COMPILERESPONSE);
      tracker.resolve(2, OutboundMessage.MessageCase.COMPILERESPONSE);
      tracker.resolve(0, OutboundMessage.MessageCase.COMPILERESPONSE);
      expect(tracker.nextId).toBe(0);
    });
    ...
    it('errors if the response type does not match what the request is expecting', () => {
      tracker.add(0, InboundMessage.MessageCase.IMPORTRESPONSE);
      expect(() =>
